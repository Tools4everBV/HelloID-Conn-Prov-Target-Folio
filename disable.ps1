##################################################
# HelloID-Conn-Prov-Target-Folio-Disable
# PowerShell V2
# Version: 1.0.0
##################################################
...
try {
    ...
    if ($null -ne $currentAccount) {
        ...
        $dryRunMessage = "Disable Folio account: [$($actionContext.References.Account)] for person: [$($personContext.Person.DisplayName)] will be executed during enforcement"
    } else {
        $action = 'NotFound'
    ...
    if (-not($actionContext.DryRun -eq $true)) {
        switch ($action) {
            'DisableAccount' {
                Write-Verbose "Disabling Folio account with accountReference: [$($actionContext.References.Account)]"
                $currentAccount | Add-Member @{
                    active = 'false'
                } -Force
                $outputContext.Data.active = 'false'
                $splatParams = @{
                    Uri         = "$($actionContext.Configuration.BaseUrl)/users/$($actionContext.References.Account)"
                    Method      = 'PUT'
                    Headers     = $headers
                    ContentType = 'application/json;charset=utf-8'
                    Body        = ([System.Text.Encoding]::UTF8.GetBytes(($currentAccount | ConvertTo-Json -Depth 10)))
                }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not disable Folio account. Error: $($errorObj.FriendlyMessage)"
        Write-Verbose "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not disable Folio account. Error: $($ex.Exception.Message)"
