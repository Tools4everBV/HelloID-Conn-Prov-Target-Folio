############################################
# HelloID-Conn-Prov-Target-Folio-Enable
# PowerShell V2
# Version: 1.0.0
############################################
...
try {
    ...
    if ($null -ne $currentAccount) {
        ...
        $dryRunMessage = "Enable Folio account: [$($actionContext.References.Account)] for person: [$($personContext.Person.DisplayName)] will be executed during enforcement"
    } else {
        $action = 'NotFound'
    ...
    if (-not($actionContext.DryRun -eq $true)) {
        switch ($action) {
            'EnableAccount' {
                Write-Verbose "Enabling Folio account with accountReference: [$($actionContext.References.Account)]"
                $currentAccount | Add-Member @{
                    active = 'true'
                } -Force
                $outputContext.Data.active = 'true'
                $splatParams = @{
                    Uri         = "$($actionContext.Configuration.BaseUrl)/users/$($actionContext.References.Account)"
                    Method      = 'PUT'
                    Headers     = $headers
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not enable Folio account. Error: $($errorObj.FriendlyMessage)"
        Write-Verbose "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not enable Folio account. Error: $($ex.Exception.Message)"
